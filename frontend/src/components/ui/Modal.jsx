...
// Legacy Modal component for backward compat
function Modal({ open, onClose, title, subtitle, children, size = 'md', footer }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose?.()}>
      <DialogContent className={cn(widths[size], 'max-h-[90vh] flex flex-col p-0 gap-0')}>
        ...
        {footer && (
          <div className="px-6 py-4 border-t border-border bg-muted/30 rounded-b-xl">{footer}</div>
        )}
      </DialogContent>
    </Dialog>
  )
