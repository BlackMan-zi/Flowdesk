# Two-Factor Authentication (MFA) — BSC Procurement
...
| **You can't reset your own MFA** | — | Deliberate. Ask another administrator (we have several). Prevents a hijacked session from quietly removing its own second factor. |
...
- **TOTP engine**: `lib/auth/totp.ts` — Node stdlib `crypto` only (HMAC-SHA1,
...
| User lost their phone | — | Another admin: Users → **Reset MFA** on their row |
...
| Everyone locked out | — | `sh /opt/bsc-procurement/scripts/disable-mfa.sh` on the server |
| Want code at every login | — | MFA Settings → set trusted days to **0** |
