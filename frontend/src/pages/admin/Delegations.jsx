...
export default function AdminDelegations() {
  ...
  const filtered = useMemo(() => {
    ...
    return delegations.filter(d => {
      const hay = [
        d.original_approver?.name, d.original_approver?.email,
        d.delegate_user?.name, d.delegate_user?.email,
        d.reason, d.is_active ? 'active' : 'returned'
  ...
  const columns = [
    { key: 'original', label: 'Original Approver', render: r => (
      <div>
        <p className="text-sm font-medium text-foreground">{r.original_approver?.name || '—'}</p>
        <p className="text-xs text-muted-foreground">{r.original_approver?.email}</p>
      </div>
    )},
    ...
    { key: 'reason', label: 'Reason', render: r => (
      <span className="text-sm text-muted-foreground truncate max-w-xs block">{r.reason || '—'}</span>
    )},
    ...
    { key: 'returned_at', label: 'Returned On', render: r => (
      <span className="text-sm text-muted-foreground">{fmt(r.returned_at)}</span>
    )},
  ...
  return (
    <div className="max-w-6xl space-y-5">
      ...
      {search && (
        <p className="text-xs text-muted-foreground -mt-2">{filtered.length} of {delegations.length} delegations</p>
      )}
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-foreground space-y-1">
              ...
              <p className="text-xs text-muted-foreground">The original approver will immediately regain their approval rights.</p>
            </div>
          </div>
          {returnError && (
            <p className="text-sm text-destructive bg-destructive/10 border border-destructive/30 rounded-lg px-3 py-2">{returnError}</p>
          )}
          <div className="flex gap-3">
            ...
            <Button variant="outline" onClick={() => { setReturnTarget(null); setReturnError('') }}>Cancel</Button>
          </div>
        </div>
      </Modal>
    </div>
